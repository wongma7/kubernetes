...
func (util *PhotonDiskUtil) CreateVolume(p *photonPersistentDiskProvisioner) (pdID string, capacityGB int, fstype string, mountOptions *string, err error) {
	...
	for parameter, value := range p.options.Parameters {
		...
		default:
			...
			return "", 0, "", nil, fmt.Errorf("Photon Controller Util: invalid option %s for volume plugin %s.", parameter, p.plugin.GetPluginName())
		}
	}
	...
	return pdID, volSizeGB, "", mountOptions, nil
}
